...
if len(os.listdir(r'C:\Users\User\Desktop\for_zaglushki\last_request_id\filer_free')) == 0:
...
while num != 0:
    ...
    num_files = sum(os.path.isfile(os.path.join(r'C:\Users\User\Desktop\for_zaglushki\last_request_id\filer_free', f)) for f in os.listdir(r'C:\Users\User\Desktop\for_zaglushki\last_request_id\filer_free'))
    ...
    list_of_files = glob.glob(
        r"C:\Users\User\Desktop\for_zaglushki\last_request_id\filer_free\*")
    ...
    where inn = '{inn}'
    and Message like '%elk_filer_connection_ladoga%'
    ...
    where topic = 'elk_filer_connection_ladoga' and partition_number = {partition};
    """
    cur.execute(sql)
    conn.commit()
    ...
    b_dir = r'\\LADOGA\xmls\GetEventDataXML.NewDLAccountData.xml'
    n_dir = fr'\\LADOGA\DummyService\rdbservice\result\GetEventDataXML.NewDLAccountData.xml'
    shutil.copy(b_dir, n_dir)
    out_gray('Файл GetEventDataXML.NewDLAccountData.xml успешно перенесен в папку result')

    b_dir = r'\\LADOGA\xmls\GetEventDataXML.NewDLAccountData_CRM_Done.xml'
    n_dir = fr'\\LADOGA\DummyService\rdbservice\result\GetEventDataXML.NewDLAccountData_CRM_Done.xml'
    shutil.copy(b_dir, n_dir)
    out_gray('Файл GetEventDataXML.NewDLAccountData_CRM_Done.xml успешно перенесен в папку result')

    b_dir = fr'\\LADOGA\xmls\GetEventDataXML.LoadRCDataCRM_Done.380f9a13-b053-4b92-ad4b-0bef516842fe.xml'
    ...
    os.remove(os.path.join(dir, f'GetEventDataXML.NewDLAccountData.xml'))
    os.remove(os.path.join(dir, f'GetEventDataXML.NewDLAccountData_CRM_Done.xml'))
