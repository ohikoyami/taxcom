...
def test_buy_cert():
    ...
    # Выбор «КриптоПро CSP» 5.0 (бессрочная)
    driver.find_element(By.XPATH,
                        "//label[contains (text(), '5.0 (бессрочная)')]/../../div/select[@class='form-control ng-pristine ng-untouched ng-valid ng-not-empty']/option[contains (text(), '1')]").click()
    # Проверка цены
    driver.find_element(By.XPATH,
                        "//p[contains(text(), '3800 ₽') and @class = 'ng-binding']")
    # Выбор «КриптоПро CSP» 5.0 (серверная)
    driver.find_element(By.XPATH,
                        "//label[contains (text(), 'серверная')]/../../div/select[@class='form-control ng-pristine ng-untouched ng-valid ng-not-empty']/option[contains (text(), '1')]").click()
    # Проверка цены
    driver.find_element(By.XPATH,
                        "//p[contains(text(), '73800 ₽') and @class = 'ng-binding']")
    # Выбор «КриптоПро Office Signature» (бессрочная)
    driver.find_element(By.XPATH,
                        "//label[contains (text(), 'Signature')]/../../div/select[@class='form-control ng-pristine ng-untouched ng-valid ng-not-empty']/option[contains (text(), '1')]").click()
    # Проверка цены
    driver.find_element(By.XPATH,
                        "//p[contains(text(), '75000 ₽') and @class = 'ng-binding']")
    # Выбор Лицензия на право использования СКЗИ «КриптоПро TSP Client»
    driver.find_element(By.XPATH,
    ...
    # Проверка цены
    driver.find_element(By.XPATH,
                        "//p[contains(text(), '76800 ₽') and @class = 'ng-binding']")
...
def test_cancel():
    ...
    directory_folder = r"C:\Users\User\Desktop\for_zaglushki\last_request_id\add_serv\cryptopro5_add_serv_5"
