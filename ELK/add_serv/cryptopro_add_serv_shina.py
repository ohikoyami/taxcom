...
if len(os.listdir(r'C:\Users\User\Desktop\for_zaglushki\last_request_id\add_serv\cryptopro_add_serv')) == 0:
...
while num != 0:
    ...
    num_files = sum(
        os.path.isfile(os.path.join(r'C:\Users\User\Desktop\for_zaglushki\last_request_id\add_serv\cryptopro_add_serv', f)) for f in
        os.listdir(r'C:\Users\User\Desktop\for_zaglushki\last_request_id\add_serv\cryptopro_add_serv'))
    ...
    list_of_files = glob.glob(
        r"C:\Users\User\Desktop\for_zaglushki\last_request_id\add_serv\cryptopro_add_serv\*")
    ...
    if corr > 0:
        ...
        if "CPLic40gLim 1" in addparams:
            print("CPLic40gLim Found")
            add_serv = add_serv + '1'
        else:
            print("CPLic40gLim Not Found")
            add_serv = add_serv + 'CPLic40gLim '
        ...
        if "CPOCSP 1" in addparams:
            print("CPOCSP Found")
            add_serv = add_serv + '1'
        else:
            print("CPOCSP Not Found")
            add_serv = add_serv + 'CPOCSP '

        if "CPRV 1" in addparams:
            print("CPRV Found")
            add_serv = add_serv + '1'
        else:
            print("CPRV Not Found")
            add_serv = add_serv + 'CPRV '
    ...
    # Перенос обрабатываемого файла в папку cryptopro
    base_dir = os.path.dirname(path.name)
    new_dir = fr"C:\Users\User\Desktop\for_zaglushki\last_request_id\cryptopro\{path.name}"
    shutil.copy(latest_file, new_dir)

    print('Заявка перенесена в папку cryptopro')
