...
if len(os.listdir(r'C:\Users\User\Desktop\for_zaglushki\last_request_id\add_serv\uc_rosreestr_add_serv')) == 0:
...
while num != 0:
    ...
    num_files = sum(
        os.path.isfile(os.path.join(r'C:\Users\User\Desktop\for_zaglushki\last_request_id\add_serv\uc_rosreestr_add_serv', f)) for f in
        os.listdir(r'C:\Users\User\Desktop\for_zaglushki\last_request_id\add_serv\uc_rosreestr_add_serv'))
    ...
    list_of_files = glob.glob(
        r"C:\Users\User\Desktop\for_zaglushki\last_request_id\add_serv\uc_rosreestr_add_serv\*")
    ...
    if corr > 0:
        ...
        for item in root.findall('.//Quals'):
            id_ = item.attrib.get('QualName')
            for child in list(item):
                addparams = child.text

        # Проверка корректности передачи дополнительных услуг в шину
        if "Qual61" == addparams:
            print("Qual61 Correct")
            add_serv = '1'
        else:
            print("Qual61 Not Found")
            add_serv = 'Qual61'
