...
if len(os.listdir(r'C:\Users\User\Desktop\for_zaglushki\last_request_id\add_serv\filer_add_serv')) == 0:
...
while num != 0:
    ...
    num_files = sum(
        os.path.isfile(os.path.join(r'C:\Users\User\Desktop\for_zaglushki\last_request_id\add_serv\filer_add_serv', f))
        for f in
        os.listdir(r'C:\Users\User\Desktop\for_zaglushki\last_request_id\add_serv\filer_add_serv'))
    ...
    list_of_files = glob.glob(
        r"C:\Users\User\Desktop\for_zaglushki\last_request_id\add_serv\filer_add_serv\*")
    ...
    if corr > 0:
        ...
        if "EDO-Package Tarif-EDO-100" in addparams:
            print("EDO-Package Correct")
            add_serv = add_serv + '1'
        else:
            print("EDO-Package Not Correct")
            add_serv = add_serv + 'EDO-Package '

        if "T_MARK Да" in addparams:
            print("T_MARK Found")
            add_serv = add_serv + '1'
        else:
            print("T_MARK Not Found")
            add_serv = add_serv + 'T_MARK '

        if "T_WebAPI Да" in addparams:
            print("T_WebAPI Found")
            add_serv = add_serv + '1'
        else:
            print("T_WebAPI Not Found")
            add_serv = add_serv + 'T_WebAPI '

        if "T_MODULE1C Да" in addparams:
            print("T_MODULE1C Found")
            add_serv = add_serv + '1'
        else:
            print("T_MODULE1C Not Found")
            add_serv = add_serv + 'T_MODULE1C '

        if "T_Stantion Да" in addparams:
            print("T_Stantion Found")
            add_serv = add_serv + '1'
        else:
            print("T_Stantion Not Found")
            add_serv = add_serv + 'T_Stantion'
    ...
    # Перенос обрабатываемого файла в папку filer
    base_dir = os.path.dirname(path.name)
    new_dir = fr"C:\Users\User\Desktop\for_zaglushki\last_request_id\filer\{path.name}"
    shutil.copy(latest_file, new_dir)

    print('Заявка перенесена в папку filer')
